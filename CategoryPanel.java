...
public class CategoryPanel extends JPanel implements MainGUI.RefreshablePanel {
    ...
    private String selectedCategory; // 跟踪当前选中的类别

    // 定义低饱和度半透明颜色
    private static final Color COLOR_ADD = new Color(76, 175, 80, 204); // 绿色 80% 透明度
    private static final Color COLOR_DELETE = new Color(244, 67, 54, 204); // 红色 80% 透明度
    private static final Color COLOR_UPDATE = new Color(33, 150, 243, 204); // 蓝色 80% 透明度
    ...
    private void deleteCategory() {
        ...
        if (selectedCategory != null) {
            ...
            if (confirm == JOptionPane.YES_OPTION) {
                ...
                categoryManager.saveCategoriesToFile(Constants.getCategoriesFilePath(currentUser.getId()));
                this.selectedCategory = null; // 清除选择
                ((MainGUI) SwingUtilities.getWindowAncestor(this)).refreshAllData();
    ...
    private void updateCategory() {
        ...
        if (oldCategory != null) {
            ...
            if (newCategory != null && !newCategory.trim().isEmpty() && !newCategory.equals(oldCategory)) {
                ...
                categoryManager.saveCategoriesToFile(Constants.getCategoriesFilePath(currentUser.getId()));
                selectedCategory = newCategory.trim(); // 更新选择
                ((MainGUI) SwingUtilities.getWindowAncestor(this)).refreshAllData();
